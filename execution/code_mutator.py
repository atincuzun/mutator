...
class CodeMutator(ast.NodeTransformer):
    ...
    def get_modified_code(self) -> str:
        ...
        code = ast.unparse(modified_tree)
        
        # Apply basic formatting improvements to make code more readable
        code = self._format_code_preserving_structure(code)
        
        return code

    def _format_code_preserving_structure(self, code: str) -> str:
        """
        Apply basic formatting to make code more readable while preserving structure.
        ...
        for i, line in enumerate(lines):
            # Handle nn.Sequential formatting
            if 'nn.Sequential(' in line:
                in_sequential = True
                # Extract the indentation level
                sequential_indent = line[:line.find('nn.Sequential(')]
                formatted_lines.append(line)
                continue
            
            if in_sequential and line.strip().endswith(')'):
                in_sequential = False
                formatted_lines.append(line)
                continue
            
            if in_sequential:
                # Format sequential arguments with proper indentation
                if line.strip() and not line.strip().startswith(')'):
                    # Add proper indentation for sequential arguments
                    formatted_line = sequential_indent + '    ' + line.strip()
                    formatted_lines.append(formatted_line)
                else:
                    formatted_lines.append(line)
            else:
                # Basic formatting for other lines
                formatted_lines.append(line)
        
        # Join lines and ensure proper spacing
        formatted_code = '\n'.join(formatted_lines)
        
        # Add spacing around class definitions and methods
        formatted_code = formatted_code.replace('class Net(nn.Module):', '\nclass Net(nn.Module):\n')
        formatted_code = formatted_code.replace('def __init__', '\n    def __init__')
        formatted_code = formatted_code.replace('def forward', '\n    def forward')
        formatted_code = formatted_code.replace('def train_setup', '\n    def train_setup')
        formatted_code = formatted_code.replace('def learn', '\n    def learn')
        
        # Ensure proper spacing after imports
        formatted_code = formatted_code.replace('import torch\nimport torch.nn as nn', 
                                               'import torch\nimport torch.nn as nn\n')
